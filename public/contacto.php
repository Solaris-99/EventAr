...
<nav class="navbar navbar-expand-lg navbar-dark bg-primary justify-content-between" id='navbar'>
    ...
    <section class='ms-auto me-auto container-xl'>
        <h1>Contacto</h1>
        <p>Contactanos a través de un formulario si tienes una duda sobre esta página, si quieres ayudar a colaborar o si has encontrado algún bug que reportar</p>
        <form action="../scripts/validate.php?origin=contact" method='POST' class='border border-light-subtle shadow rounded p-4'>
        <?php
...
?>
                ...
                <label for="motive">
                    Motivo
                    <input type="text" name='motive' class='border rounded mr-sm-2 d-inline p-2' required>
                </label>
                <p>Deja un mensaje...</p>
                <textarea name="ebody" id="ebody" cols="30" rows="10" default='Escribe un mensaje...' class='w-100 border rounded mr-sm-2 d-inline p-2 resize-none'></textarea>
                <input type="submit" class='btn btn-primary mt-4 mb-4 d-block'>
